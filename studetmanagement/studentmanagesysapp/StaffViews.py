...
from studentmanagesysapp.models import Subjects, SessionYearModel, Students, Attendance, AttendanceReport, \
...
def staff_add_result(request):
    ...
    #Begining of views for add exams
def add_exams_template(request):
    subjects=Subjects.objects.filter(staff_id=request.user.id)
    session_years=SessionYearModel.object.all()
    return render(request,"staff_template/add_exam_template.html",{"subjects":subjects,"session_years":session_years})
        

def staff_add_exams(request):
    subjects=Subjects.objects.filter(staff_id=request.user.id)
    session_years=SessionYearModel.object.all()
    return render(request,"staff_template/add_exam.html",{"subjects":subjects,"session_years":session_years})


def staff_view_exams(request):
    subjects=Subjects.objects.filter(staff_id=request.user.id)
    session_years=SessionYearModel.object.all()
    return render(request,"staff_template/staff_view_exams.html",{"subjects":subjects,"session_years":session_years})
    

def add_question_template(request):
    subjects=Subjects.objects.filter(staff_id=request.user.id)
    session_years=SessionYearModel.object.all()
    return render(request,"staff_template/add_question_template.html",{"subjects":subjects,"session_years":session_years})

def staff_view_questions(request):
    subjects=Subjects.objects.filter(staff_id=request.user.id)
    session_years=SessionYearModel.object.all()
    return render(request,"staff_template/staff_view_questions.html",{"subjects":subjects,"session_years":session_years})

def staff_add_question(request):
    subjects=Subjects.objects.filter(staff_id=request.user.id)
    session_years=SessionYearModel.object.all()
    return render(request,"staff_template/add_question.html",{"subjects":subjects,"session_years":session_years})

#Note that this is the quiz views
